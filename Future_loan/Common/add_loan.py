...
param_data=Do_Excel().read_Excel('add_loan')#读取测试数据

@ddt
class Add_Loan(unittest.TestCase):
    ...
    def test_login(self,param_data):
        ...
        # if param_data["Params"].find('loanid')!=-1:
        ...
        try:
            if param_data["Sql"]!=None:
                LOANID=Mysql().do_Mysql(eval(param_data["Sql"])["sql"])
                setattr(Get_attribute,'loanid',str(LOANID[0]))
        except Exception as e:
            TestLog().error('错误是：{}'.format(e))
        ...
        #
        # finally:
        #     Do_Excel().write_excel('add_loan',param_data["Case_Id"]+1,9,actual.text)#把请求的响应结果写回Excel中
        #     Do_Excel().write_excel('add_loan',param_data["Case_Id"]+1,10, TestRes)#把比对之后的结论写回Excel中
